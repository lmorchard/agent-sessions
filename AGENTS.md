...
- **An autonomy harness with a skill component.** The system's orchestration lives in `driver/`; the skill component lives at `skills/agent-session/` in *this repo* — it is NOT installed in `~/.Codex/skills/`. Test it by running its phase files manually (dogfooding), not via a registered skill.
