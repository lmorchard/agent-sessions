...
def run_classify_only(ctx: RunContext) -> int:
    ...
    if not matching_pr_dict:
        ...
        failed_ci, _ = check_pr_ci_status(ctx.repo, pr_num)
    ...
    apply_park_state(issue_num, outcome, ts, reason, ctx.repo, ctx.state_dir, ctx.parked_log)
...
def select_queue(ctx: RunContext) -> SelectionResult:
    say("== select ==")
    board_items = fetch_board_json(ctx.board) if ctx.board and not ctx.all_issues else []
    ...
    for iss in all_issues_json:
        ...
        if n in parked_nums and n != ctx.retry:
            park_t = get_park_time(n, ctx.state_dir)
            updated_at = open_issues_map.get(n, "")
            has_human, login = has_new_human_comment(n, ctx.repo, ctx.driver_bots, park_time=park_t, issue_updated_at=updated_at)
            human_comments_map[n] = (has_human, login)
            if not has_human:
                park_reasons[n] = park_reason(n, ctx.state_dir)
        attempts_map[n] = get_attempts(n, ctx.repo, issues_json=all_issues_json)
    ...
    for m in sel_res["unpark_actions"]:
        park_label_remove(m, ctx.repo)

    for m, reason in sel_res["park_actions"]:
        apply_park_state(m, "parked", ts_str, f"parked by loop breaker: {reason}", ctx.repo, ctx.state_dir, ctx.parked_log, quiet=True)
    ...
    for cand_item in all_candidates:
        ...
        if acquire_lock(num, cand_phase, ctx.repo_path):
...
def invoke_agent(
    ...
    increment_attempts(issue_num, ctx.repo)
    ...
    if prline:
        ...
        try:
            ...
            if res.returncode == 0:
                ...
                unresolved_text = get_pr_unresolved_threads_text(ctx.repo, prnum, ctx.creds.read_token)
                if unresolved_text:
    ...
    prompt = build_prompt(url, phase, ctx.skill_dir, writes_file, extra_context=extra_context)
    ...
    if phase == "execute" and ctx.board and issue_num in board_item_ids:
        if mark_board_in_progress(ctx.board, board_item_ids[issue_num]):
    ...
    writes_result = perform_writes(writes_file, ctx.repo, run_repo_path, rundir, ctx.board)
...
def classify_and_record(
    ...
    if inv.exit_code == 124:
        ...
        if not prline:
            if inv.phase in ("triage", "refine"):
                ...
                if PARK_LABEL in lbls or INTERACTIVE_LABEL in lbls:
                    ...
                    if "inconclusive" in inv.final_text.lower():
                        ...
                        decrement_attempts(inv.issue_num, ctx.repo)
                    else:
            ...
            if matching_pr_dict and "statusCheckRollup" in matching_pr_dict:
                ...
                failed_ci, _ = check_pr_ci_status(ctx.repo, prnum)
            ci_checks = "failed" if failed_ci > 0 else "pass"
            outcome_res = gate.classify(
                pr_body,
                head_sha=head_sha,
    ...
    write_note = writes_summary(inv.writes_result)
    if write_note:
    ...
    apply_park_state(inv.issue_num, outcome, inv.ts, reason, ctx.repo, ctx.state_dir, ctx.parked_log)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if ctx.classify_only:
        return run_classify_only(ctx)

    sel_res = select_queue(ctx)
    ...
    for num, phase in sel_res.candidates:
        ...
        run_repo_path = prepare_workspace(ctx, num)
        inv = invoke_agent(ctx, num, phase, run_repo_path, sel_res.open_prs, sel_res.board_item_ids)
        run_outcome = classify_and_record(ctx, inv, sel_res.open_prs)
    ...
    report_results(summary_rows, attempted, total_cost, ctx.state_dir)
    return 0
