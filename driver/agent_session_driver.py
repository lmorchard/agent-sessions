...
def acquire_lock(issue_number: str | int, phase: str, repo_path: Path) -> bool:
    ...
    try:
        ...
        lock_sha = res.stdout.decode("utf-8").strip()
    except Exception:
        return False
    ...
    try:
        ...
        lock_time = int(res.stdout.strip() or "0")
    except Exception:
        lock_time = 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.classify_only:
        ...
        if not matching_pr_dict:
            ...
            pr_num = matching_pr_dict.get("number")
    ...
    all_candidates = p1_unblock + p2_execute + p3_groom + p4_escalate
    ...
    for item in all_candidates:
        num, phase = item
        if acquire_lock(num, phase, repo_path):
