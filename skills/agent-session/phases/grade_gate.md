...
1. **Derive the verdict.** Not a judgment call — read each row and take the result.
    ...
    *(For a small task there is no freeze commit, so the tamper and freeze rows are `none` and the
    ...
    | Condition | Source |
    |---|---|
    | Every criterion with a check: `pass` | The independent verifier's report — `execute`'s, or `open_pr`'s if it re-ran after the rebase |
    ...
    | Tamper diff clean, or every difference logged as an amendment | The verdict `open_pr` recorded when it ran the tamper check — and still re-runnable, since the freeze commit is an ancestor of the pushed head. `clean-by-substitute` counts, bare `clean` on an empty `Check files` list does not |
    ...
    | PR touches no risk-gated path | The diff vs. **this project's** risk list, in its instruction file (`CLAUDE.md`, or `AGENTS.md` where that is what the project keeps). `acceptance-criteria.md`'s list is the *generic default*, used only where the project names none |
...
Unfinished work and red checks are preconditions, not skip conditions: fix them (`fix_ci` for red
checks, `address_comments` for review threads), then run `grade_gate` again.
